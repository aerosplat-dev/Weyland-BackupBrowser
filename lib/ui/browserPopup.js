 ...
 * @property {(signal?: AbortSignal) => Promise<object>} acquireSession
...
export async function openBackupBrowser(deps) {
    ...
    root.append(
        el('p', {
            ...
            text: t`Only the latest 50 saves per character are kept. Restore what you need before chatting again.`,
        }),
        statusBox,
        body);
    ...
    function renderCharacters() {
        ...
        characterList.replaceChildren(...state.characters.map((entry) => {
            ...
            const meta = newest
                ? `${entry.backups.length === 1 ? t`1 backup` : t`${entry.backups.length} backups`} · ${formatTime(newest.mtime)}`
                : t`No backups left`;
    ...
    function renderChats() {
        ...
        // replaceChildren() would render a null argument as the text "null", so filter it out.
        chatsHeader.replaceChildren(...[
        ...
        if (pending.length > 0) sections.push(el('p', { className: 'wbb-pending', text: pending.length === 1 ? t`1 backup still to read…` : t`${pending.length} backups still to read…` }));
    ...
    async function loadSession() {
        try {
            ...
            state.error = null;
            render();
            const session = await deps.acquireSession();
            if (state.closed) return;
    ...
    async function selectCharacter(entry) {
        ...
        deps.chatsApi.listCurrentChats(entry.avatar, signal)
            ...
            .catch(() => {
                if (signal.aborted) return;
                console.warn(`${LOG_PREFIX} Couldn't list ${entry.name}'s current chats.`);
    ...
    async function handleRestore(entry, group, snapshot) {
        ...
        if (needsConfirmation(group, state.rows)) {
            const armedAt = state.confirming.get(hash);
            if (armedAt === undefined) {
                state.confirming.set(hash, Date.now());
                render();
                return;
            }
