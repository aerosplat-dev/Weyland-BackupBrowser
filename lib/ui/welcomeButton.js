...
/**
 * Keeps an icon-only Backups button in the welcome panel's shortcut row, right after Temporary Chat.
 ...
 * @param {{chatElement: HTMLElement, title: string, onClick: () => void}} options
 * @returns {{destroy: () => void}}
 */
export function installWelcomeButton({ chatElement, title, onClick }) {
    function addMissingButtons() {
        for (const shortcuts of chatElement.querySelectorAll('.welcomePanel .welcomeShortcuts')) {
            ...
            const button = iconButton({ icon: 'fa-box-open', title, className: WELCOME_BUTTON_CLASS, onClick });
