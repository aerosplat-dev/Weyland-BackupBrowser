import { compactText, formatCreateDate, formatSize, formatSnapshotTime } from '../format.js';
...
function statusText(t, group, existingFailed) {
    switch (group.status) {
        case GROUP_STATUS.RESTORED: return t`Restored`;
        case GROUP_STATUS.EXISTS: return t`Exists`;
        case GROUP_STATUS.DELETED: return t`Deleted`;
        default: return existingFailed ? t`Status unknown` : t`Checking…`;
    }
}

function messageCount(t, count) {
    return count === 1 ? t`1 message` : t`${count} messages`;
}
...
function snapshotRow(options, group, snapshot, latest) {
    ...
    const meta = [formatSnapshotTime(snapshot.mtime), messageCount(t, summary.messageCount)];
    ...
    return el('div', { className: latest ? 'wbb-snapshot wbb-snapshot-latest' : 'wbb-snapshot' },
        ...
        el('span', { className: 'wbb-actions' },
            iconButton({
                icon: 'fa-eye',
                label: t`Preview`,
                title: t`Preview this backup`,
                className: 'wbb-preview-button',
                onClick: () => options.onPreview(group, snapshot),
            }),
            iconButton({
                icon: 'fa-rotate-left',
                label: confirming ? t`Confirm restore` : t`Restore`,
                title: t`Restore as a new chat`,
                className: confirming ? 'wbb-restore-button wbb-confirming' : 'wbb-restore-button',
                disabled: options.isQueued(snapshot.hash),
                onClick: () => options.onRestore(group, snapshot),
            })),
        latest && summary.lastMessage
            ? el('div', { className: 'wbb-excerpt', text: `${summary.lastMessage.name}: ${compactText(summary.lastMessage.excerpt)}` })
            : null,
...
export function groupSection(options, group) {
    ...
    const section = el('section', { className: 'wbb-group', attrs: { 'data-status': group.status } },
        el('header', { className: 'wbb-group-header' },
            ...
            group.fileName
                ? el('span', { className: 'wbb-group-file', text: group.fileName, attrs: { title: group.fileName } })
                : null),
        snapshotRow(options, group, latest, true));
    if (older.length > 0) {
        section.append(iconButton({
            icon: expanded ? 'fa-chevron-up' : 'fa-chevron-down',
            label: older.length === 1 ? t`1 older snapshot` : t`${older.length} older snapshots`,
    ...
    return el('section', { className: 'wbb-group wbb-group-problems' },
        ...
        ...problems.map((snapshot) => el('div', { className: 'wbb-snapshot' },
            el('span', {
                ...
                text: [
                    formatSnapshotTime(snapshot.mtime),
