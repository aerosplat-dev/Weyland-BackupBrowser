...
/** Restore asks for confirmation unless the chat is known to be gone and nothing was restored from it yet. */
export function needsConfirmation(group, rows) {
    if (group.status !== GROUP_STATUS.DELETED) return true;
    return group.snapshots.some((snapshot) => rows.get(snapshot.hash)?.kind === 'restored');
}
